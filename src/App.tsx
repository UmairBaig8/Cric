    ...
    <div className={dark ? 'app dark' : 'app'}>
      ...
      <main id="top">
        <section className="hero" id="tournament">
          ...
          <ElectricBorder className="hero-flame" color="#00E5FF" speed={2.2} chaos={0.34} borderRadius={20}><aside className="hero-panel"><div className="panel-label">DPL 2026 / REGISTRATION WINDOW</div><div className="panel-title">Your spot is waiting.</div><div className="countdown"><div className="time"><b>{time(days)}</b><span>DAYS</span></div><div className="time"><b>{time(hours)}</b><span>HRS</span></div><div className="time"><b>{time(mins)}</b><span>MINS</span></div><div className="time"><b>{time(secs)}</b><span>SECS</span></div></div><div className="capacity"><div className="capacity-head"><span>PLAYER CAPACITY</span><strong><span>{playerCount}</span> / {capacity}</strong></div><div className="meter"><i style={{ width: `${meterWidth}%` }} /></div><div className="spots">Only <strong>{spotsLeft} spots</strong> left. Don&apos;t sit this one out.</div></div><Magnet magnetStrength={3} padding={90}><a className="btn btn-primary panel-cta" href="/D2P/register">YES, COUNT ME IN →</a></Magnet></aside></ElectricBorder>
        </section>
        ...
        <section className="desk-pitch" id="journey"><div className="shell dp-glass"><Reveal><div className="section-head"><div className="eyebrow cyan">THE D2P JOURNEY</div><h2>FROM DESK.<br /><span>TO PITCH.</span></h2><p>Same colleagues. Different battlefield. The transformation that makes D2P what it is.</p></div></Reveal><Reveal delay={0.12}><div className="dp-track"><article className="dp-card desk"><div className="dp-icon">💻</div><div className="dp-time">9:00 AM</div><h3>THE DESK</h3><p>Emails · Meetings · Code · Deadlines</p></article><div className="dp-arrow" aria-hidden="true">→</div><article className="dp-card pitch"><div className="dp-icon">🏏</div><div className="dp-time">7:00 PM</div><h3>THE PITCH</h3><p>Teams · Sixes · Rivalries · Glory</p></article></div></Reveal><Reveal delay={0.2}><p className="dp-tagline">Same colleagues. <span>Different battlefield.</span></p></Reveal></div></section>

        <section className="squad" id="players"><div className="shell"><Reveal><div className="section-head"><div className="eyebrow">ALREADY IN</div><h2>MEET THE SQUAD.</h2><p>These players have locked in their spot. Your player card could be next.</p></div></Reveal><Reveal delay={0.12}>{playerCards.length ? <div className="player-grid">{playerCards.map((player) => <article className="player-card" key={player.id}><div className="player-photo">{player.photo_url ? <img alt={player.name} src={player.photo_url} /> : <span>{player.name.split(' ').map((word) => word[0]).slice(0, 2).join('')}</span>}</div><h3>{player.name}</h3><span className="player-role">{player.player_type}</span><span className="player-squad">{player.location}</span></article>)}</div> : <div className="player-grid">{[1, 2, 3, 4, 5, 6, 7, 8].map((slot) => <article className="player-card placeholder" key={slot}><div className="player-photo"><span>?</span></div><h3>YOUR NAME</h3><span className="player-role">YOUR ROLE</span><span className="player-squad">CZ · SP · OTHER</span></article>)}</div>}</Reveal><Reveal delay={0.2}><div className="squad-cta"><Magnet magnetStrength={3} padding={90}><a className="btn btn-primary" href="/D2P/register">🏏 ADD YOUR CARD →</a></Magnet></div></Reveal></div></section>

        <section className="section" id="how"><div className="shell"><Reveal><div className="section-head"><div className="eyebrow">FROM SIGN-UP TO TROPHY</div><h2>HOW DPL WORKS</h2><p>One simple journey from your desk to match day. Your player profile follows you all the way to the final.</p></div></Reveal><Reveal delay={0.12}><div className="steps">{steps.map(([icon, title, copy]) => <article className="step" key={title}><div className="step-dot">{icon}</div><h3>{title}</h3><p>{copy}</p></article>)}</div></Reveal></div></section>

        <section className="benefits" id="teams"><div className="shell"><Reveal><div className="section-head"><div className="eyebrow cyan">THE OFFICE. THE TEAMS. THE STORY.</div><h2>DPL IS MORE THAN CRICKET.</h2><p>It&apos;s the part of the year where the person sitting two desks away suddenly becomes your biggest rival.</p></div></Reveal><Reveal delay={0.12}><div className="benefit-grid"><article className="benefit"><div className="num">01</div><h3>MEET YOUR OTHER TEAM.</h3><p>Forget the org chart for a day. Developers, designers, sales, HR, managers — everyone starts on the same pitch.</p><div className="team-cloud">{teams.map((team) => <span className="team" key={team}>{team}</span>)}</div></article>{benefits.map(([number, title, copy, id]) => <article className="benefit" id={id} key={number}><div className="num">{number}</div><h3>{title}</h3><p>{copy}</p></article>)}</div></Reveal></div></section>

        <section className="cta" id="register"><div className="shell"><Reveal><div className="eyebrow">YOUR NEXT OFFICE STORY</div><h2>DON&apos;T JUST WORK<br /><span>PLAY TOGETHER.</span></h2><p>Registration takes a minute. The memories last much longer.</p><Magnet magnetStrength={3} padding={90}><a className="btn btn-primary" href="/D2P/register">🏏 YES, COUNT ME IN →</a></Magnet></Reveal></div></section>
      </main>
