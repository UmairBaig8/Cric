    ...
    <style
      dangerouslySetInnerHTML={{
        __html: Object.entries(THEMES)
          .map(([theme, prefix]) => `@layer base {${prefix} [data-chart=${id}] {${colorConfig
            .map(([key, itemConfig]) => {
              ...
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  {itemConfig?.icon ? (
                    ...
                    !hideIndicator && (
                      <div className={cn('shrink-0 rounded-[2px] border-(--color-border) bg-(--color-bg)', indicator === 'dot' && 'h-2.5 w-2.5', indicator === 'line' && 'w-1', indicator === 'dashed' && 'w-0 border-[1.5px] border-dashed bg-transparent', nestLabel && indicator === 'dashed' && 'my-0.5')} style={{ '--color-bg': indicatorColor, '--color-border': indicatorColor } as React.CSSProperties} />
                    )
                  )}
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: TooltipItem | undefined, key: string) {
  if (!payload) return undefined;
  const payloadPayload = payload.payload;
  let configLabelKey: string = key;
  if (key in payload && typeof payload[key as keyof TooltipItem] === 'string') {
    configLabelKey = payload[key as keyof TooltipItem] as string;
